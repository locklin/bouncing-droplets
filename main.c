/*
 * Bouncing Droplet Simulator — Level 0 (Direct Bessel)
 *
 * Discrete dynamical map: a droplet bounces inside a corral.
 * Each bounce deposits a wave J_0(k_F * r) that decays by mu per bounce.
 * The droplet is kicked by the gradient of the accumulated wave field.
 * Wave field is maintained incrementally: O(GRID^2) per bounce.
 *
 * Accumulates wave field moments <h^2> and <h^4> for testing the
 * Born rule and Khrennikov's 4th-order correction hypothesis.
 *
 * Controls:
 *   Space       pause / resume
 ...
 *   G           cycle geometry
 *   V           cycle right panel view (histogram / Born / kurtosis)
 *   T           toggle turbo mode
 *   R           reset simulation
...
/* Incremental wave field update: decay + add new bounce source */
...
{
    /* Gradient from memory buffer (analytic, exact) */
    double gx = 0, gy = 0;
    ...
    /* Wave field update */
    wave_update(drop.x, drop.y);

    /* Histogram (Gaussian splat) */
...
static void render_right_tex(Texture2D tex)
{
    if (view_mode == VIEW_HIST) {
        ...
        for (int iy=0; iy<GRID; iy++)
            for (int ix=0; ix<GRID; ix++) {
                double v = h2_sum[iy][ix]/moment_n;
                if (v > peak) peak = v;
            }
...
int main(void)
{
    InitWindow(WIN_W, WIN_H, "Bouncing Droplet - Level 0");
    SetTargetFPS(30);
    ...
    while (!WindowShouldClose()) {
        ...
        if (IsMouseButtonPressed(MOUSE_BUTTON_LEFT) && !turbo) {
            ...
            if (m.x>=GAP && m.x<GAP+PANEL && m.y>=GAP && m.y<GAP+PANEL) {
                ...
                if (geo_inside(px,py)) { drop.x=px; drop.y=py; }
            }
        }
        ...
        snprintf(buf,sizeof(buf),
            "kF: %.1f [K/L]  |  %s [G]  |  Samples: %d  |  [T] turbo  |  [R] reset",
            P.kF,geo_name(),moment_n);
